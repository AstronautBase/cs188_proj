...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def maximize(self, gameState, depth, numGhosts):
        ...
        for action in actions:
            ...
            tmpVal = self.minimize(successor, depth, 1, numGhosts)
            if tmpVal > maxVal:
                maxVal = tmpVal
                best_action = action
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def maximize(self, gamestate, depth, numGhosts, alpha, beta):
        ...
        for action in actions:
            successor = gamestate.generateSuccessor(0, action)
            tmpVal = self.minimize(successor, depth, 1, numGhosts, alpha, beta)
            if tmpVal > maxVal:
                maxVal = tmpVal
                best_action = action
            if maxVal > beta:
    ...
    def minimize(self, gameState, depth, agentIndex, numGhosts, alpha, beta):
        ...
        for action in gameState.getLegalActions(agentIndex):
            ...
            if agentIndex == numGhosts:
                if depth < self.depth:
                    tmpVal = self.maximize(successor, depth + 1, numGhosts, alpha, beta)
                else:
                    tmpVal = self.evaluationFunction(successor)
            else:
                tmpVal = self.minimize(successor, depth, agentIndex + 1, numGhosts, alpha, beta)
            if tmpVal < minVal:
                minVal = tmpVal
